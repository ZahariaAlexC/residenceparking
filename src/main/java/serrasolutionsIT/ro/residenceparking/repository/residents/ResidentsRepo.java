...
public interface ResidentsRepo extends CrudRepository<Residents, Integer> {
    ...
    @Query(value = "select residents.id,first_name, last_name, username, block, scale, apartment, registration_number, residents.date, residents_cars from residents inner join residents_cars on residents.residents_cars = residents_cars.id where username = :username", nativeQuery = true)
